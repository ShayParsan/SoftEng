...
public class ScheduleService {
    ...
    public ScheduleService(ScheduleRepository scheduleRepository, CourseRepository courseRepository, LecturerRepository lecturerRepository, StudentRepository studentRepository, RoomRepository roomRepository) {
    ...
    public Schedule assignRoom(Long scheduleId, Long roomId) {
        ...
        var overlapping = scheduleRepository.findAll().stream()
            .anyMatch(s ->
                s.getRoom() != null &&
                s.getRoom().getId().equals(roomId) &&
                !s.getId().equals(scheduleId) &&
                !(s.getEnd().isBefore(schedule.getStart()) || s.getStart().isAfter(schedule.getEnd()))
            );
    
        if (overlapping) {
            throw new CoursesException("Room is already assigned to another schedule at this time");
        }
